...
class InstructionOptimizer:
    ...
    def __init__(
        ...
        # 加载优化配置
        self.optimized_config = self._load_optimization_config(config_path)
        logger.info(f"加载优化配置: {json.dumps(self.optimized_config, indent=2) if self.optimized_config else '使用默认配置'}")
    ...
    def _call_qwen_analysis(self, user_input: str) -> Dict[str, Any]:
        ...
        if self.optimized_config and "optimized_model" in self.optimized_config:
            optimized_model = self.optimized_config["optimized_model"]
            if optimized_model:
                model_to_use = optimized_model
                logger.info(f"使用优化模型: {model_to_use}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                options = {
                    "temperature": 0.1,
                    "num_predict": 256  # 减少预测长度，提高响应速度
                }

                if self.optimized_config and "options" in self.optimized_config:
                    options.update(self.optimized_config["options"])

                # 设置超时
                timeout = self.optimized_config.get("timeout", 30) if self.optimized_config else 30

                # 根据重试次数调整超时
                if attempt > 0:
                    timeout = min(timeout * (attempt + 1), 60)  # 最大60秒
                ...
                if response.status_code == 200:
                    ...
                    if "eval_duration" in result:
                        eval_time = result["eval_duration"] / 1e9  # 纳秒转秒
                        logger.debug(f"模型推理时间: {eval_time:.3f}s")

                    return self._parse_qwen_response(response_text)
                else:
                    logger.warning(f"Qwen API调用失败 (尝试 {attempt + 1}/{max_retries}): {response.status_code}")
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error(f"所有重试失败: {response.status_code}")
                        return self._mock_qwen_analysis(user_input)
                ...
                logger.error(f"Qwen API调用异常 (尝试 {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    wait_time = retry_delay * (backoff_factor ** attempt)
                    logger.info(f"等待 {wait_time:.1f}秒后重试...")
                    time.sleep(wait_time)
                    ...
                    logger.error(f"所有重试异常: {e}")
                    return self._mock_qwen_analysis(user_input)

        # 理论上不会执行到这里，但为了安全返回模拟结果
        return self._mock_qwen_analysis(user_input)

    def _load_optimization_config(self, config_path: str = None) -> Dict[str, Any]:
        """
        加载优化配置
        ...
        if config_path is None:
            ...
            default_paths = [
                Path("config.yaml"),
                Path("ollama_text2loc_config.json"),
                ...
                if config_path.endswith('.yaml') or config_path.endswith('.yml'):
                    with open(config_path, 'r', encoding='utf-8') as f:
                        ...
                        return {
                            ...
                            "model": ollama_config.get("model", "qwen3-vl:2b"),
                            "optimized_model": ollama_config.get("optimized_model", None)
                        }
